...
class BreadthFirstSearch<T> {
	...
	/**
	 * perform a BreathFirstSearch in an acyclic graph represented by a root node and an exploration relationship
	 * and return the first matching node
	 * 
	 ...
	 */
	public T find(T rootNode, BiPredicate<T, Collection<T>> criterion, Function<T, Collection<T>> getParents) {
	...
	public void scan(T rootNode, BiConsumer<T, Collection<T>> consumer, Function<T, Collection<T>> getParents) {
		Queue<T> queue = new ArrayDeque<>();
		queue.add(rootNode);
		while (!queue.isEmpty()) {
			T current = queue.remove();
